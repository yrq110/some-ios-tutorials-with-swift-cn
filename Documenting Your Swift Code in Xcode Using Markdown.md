...
撰写代码文档对每个开发者来说都是一项重要的任务，虽然看上去这会拖慢开发的进度，不过的确是开发中的一部分。我不觉得为每一个属性、函数、类、结构体与其他项目中存在的项撰写合适的、便于理解的文档是件容易的事情。可以通过以下要点来书写文档：

* 描述属性、函数与类的作用、目的、细节等，高亮函数特定的条件、用例或者规则。
* 高亮方法的输入与输出(参数与返回值)。
...
下面来分析一下Jazzy的具体实现细节，可以看看GitHub上的介绍，也能看到它的环境要求与安装方法，只需要如下操作即可开始使用:
...
为了方便测试Jazzy，我准备了一个小型app，可以在[这里](https://github.com/appcoda/SwiftDocSample)下载，包含之前例子中的函数-组合与拆分fullname。
